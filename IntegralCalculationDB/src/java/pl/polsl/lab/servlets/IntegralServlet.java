...
public class IntegralServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            try {
                ...
                out.println("Calculated integral is equal to " + result);
                                
                updateHistory(nodes, values, result, response);
                
                
                response.addCookie(new Cookie("lastCalculation", String.valueOf(result)));
                ...
                response.sendError(HttpServletResponse.SC_BAD_REQUEST, 
                    "Wrong input data. Type an array of double values using  a ' ' as a separator.");
            }
    ...
    /**
     * Counter of calculations that have been performed and added to database
     */

    /**
     * Adds new record to the history of calculations
     * @param nodes vector of nodes
     * @param values vector of corresponding values
     * @param result the result
     */
    private void updateHistory (Vector<Double> nodes, Vector<Double> values, Double result,
        ...
        try (Connection con = DriverManager.getConnection("jdbc:derby://localhost:1527/integration", "lab", "pass")) {
            ...
            for (int i = 0; i < nodes.size(); i++) {
                pointString += "(" + nodes.get(i) + ", " + values.get(i) + "); ";
            }

            Statement statSelect = con.createStatement();

            ResultSet rs = statSelect.executeQuery("SELECT * FROM Calculations ORDER BY id DESC");
...
//            }
            ...
            Statement statUpdate = con.createStatement();
            
            statUpdate.executeUpdate("INSERT INTO Calculations VALUES ("
                    +  newId + ", '" + pointString + "', " + result + ")");
